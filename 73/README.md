## Experiment 73: Adding Startup Scripts, Continued

#### New to Me
- We need to add the script that will start up nodemon in watch mode:
```json
"watch": "npx nodemon --watch src --ext js,ts,jsx,tsx --exec \"./sh/dev.sh\""
```
- This watches for changes to .js, .ts, .jsx, or .tsx files inside the `src` folder. When a change is detected, it runs the shell script file `dev.sh`.
